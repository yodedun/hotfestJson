...
(function($) {
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		prevMonth2 = d.getMonth() == 0 ? new Date(d.getFullYear()-1, 11, 1) : new Date(d.getFullYear(), d.getMonth()-1, 1);
		nextMonth2 = d.getMonth() == 11 ? new Date(d.getFullYear()+1, 0, 1) : new Date(d.getFullYear(), d.getMonth()+1, 1);
		...
		//	return false;  
		//});
		//$$('#calendario').swipeRight(function() {
		...
		for (var i=defaults.firstDayOfWeek; i<defaults.firstDayOfWeek+7; i++) {
			...
			headRow.append('<th title="' + wordday + '" class="DateHeader' + (weekday == 0 || weekday == 7 ? ' Weekend' : '') + '"><span>' + wordday + '</span></th>');
		}
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				var atts = {'class':"DateBox" + (weekday == 0 || weekday == 7 ? ' Weekend ' : ''),
                    ...
                    dateBox.droppable({
                        ...
                        drop: function(e, ui) {
                            ...
                            $.each(calendarEvents, function() {
                                if (this.EventoId == ui.draggable.attr("id")) {
                                    event = this;
                                }
                            });
	...
	var DrawEventsOnCalendar = function() {	
		if (calendarEvents && calendarEvents.length > 0) {
			...
			jQuery.each(calendarEvents, function(){
				...
				if(ev.Fecha) {
					if (typeof ev.Fecha == 'object' && ev.Fecha.getDate) { sDt = ev.Fecha; }
					if (typeof ev.Fecha == 'string' && ev.Fecha.split) { sDt = GetJSONDate(ev.Fecha); }
				...
				if(sDt) {
					if ((sDt >= _beginDate) && (sDt <= _endDate)) {			
						...
						label.addClass( eveN );
						//label.css( {"background-color" : ev.Color  + ""} );//relleno
						event.fadeIn(50);
					}
				}
			});
